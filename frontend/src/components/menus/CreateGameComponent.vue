<template>
    <div class="createGame">
        <div v-if=!editing class="tittle">
            <div
                style="position: absolute; font-size: 25px; left: 50%; top: 1%; transform: translateX(-50%); -webkit-text-stroke-width: 1.5px; -webkit-text-stroke-color: black; color: white;">
                New Game
            </div>
            <div class="type_div">
                <div>Type:</div>
                <button class="buttons_attribute type_div_buttons" :style="{ 'border': getSelected('type', 1) || 'none' }"
                    @click="changeTypeCurrent(1)">
                    Solo</button>
                <button class="buttons_attribute type_div_buttons" style="left: 48%;"
                    :style="{ 'border': getSelected('type', 2) || 'none' }" @click="changeTypeCurrent(2)">
                    Multiplayer</button>
            </div>
            <div class="type_div" style="top: 35%;">
                <div>Score:</div>
                <button class="buttons_attribute type_div_buttons" style="left: 30%;"
                    :style="{ 'border': getSelected('score', 3) || 'none' }" @click="changeScoreCurrent(3)">
                    3</button>
                <button class="buttons_attribute type_div_buttons" style="left: 45%;"
                    :style="{ 'border': getSelected('score', 6) || 'none' }" @click="changeScoreCurrent(6)">
                    6</button>
                <button class="buttons_attribute type_div_buttons" style="left: 60%;"
                    :style="{ 'border': getSelected('score', 9) || 'none' }" @click="changeScoreCurrent(9)">
                    9</button>
                <button class="buttons_attribute type_div_buttons" style="left: 75%;"
                    :style="{ 'border': getSelected('score', 12) || 'none' }" @click="changeScoreCurrent(12)">
                    12</button>
            </div>
            <!-- <div>View:</div> -->
        ...
        <div class="type_div" style="height: 30% ; top: 50%">
            ...
            <div class="table_image" :style="{ 'background-color': selectedColor }">
                <img v-if="asDefaultTable()" id="tableImage" :src="getTable()" class="user_paddle_image"
        ...
        <div v-if=editing class="tittle">
            ...
            <button class="buttons_attribute type_div_buttons"
                style="position: absolute; top: 90% ; left: 50%; transform: translateX(-50%);" :disabled="isChange()"
                @click="saveTable()">
                Save</button>
        </div>
        <button class="buttons_attribute type_div_buttons" style="top: 80% ; left: 50%; transform: translateX(-50%);"
            @click="changeForEdit()">
