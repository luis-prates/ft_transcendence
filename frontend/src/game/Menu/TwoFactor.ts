...
export class TwoFactor {
  ...
  constructor() {
    if (!userStore().user.isTwoFAEnabled) this.getQrImage();
    this.qrImage.src = "";
  ...
  private createBackground(): ElementUI {
    ...
    const background: ElementUI = {
      ...
      draw: (ctx: any) => {
        ...
        if (this.qrImage.complete) ctx.drawImage(this.qrImage, pos.x + pos_image_x, image_y, lado, lado);

        ctx.strokeStyle = "black";
        ctx.lineWidth = 3;
        if (!userStore().user.isTwoFAEnabled) ctx.strokeRect(pos.x + pos_image_x, image_y, lado, lado);
      },
    };
    return background;
  }
